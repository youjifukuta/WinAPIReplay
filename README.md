...
├─ LICENSE-DATA           CC BY 4.0 — applies to data/, results/ and figures/ (not to sample_identifiers/)
...
├─ figures/               the 8 figures of the paper, as PDF (Figures 1–8; see section 8)
├─ scripts/
│    ├─ python/           post-processing scripts (see section 6)
│    │    ├─ derive_table5_7_10.py   re-derives the three table5/7/10 CSV files below (written for v1.0.1)
...
└─ results/
     ├─ csv/              21 aggregated result files (CSV + sideeffect_raw.json), including
     │                    table5_bias_by_api.csv, table7_substitute_by_group.csv and
     │                    table10_intercepted_by_category.csv (added in v1.0.1)
     └─ figures/          12 files — 5 PNG figures and 7 XLSX tables, as written by the scripts
```
...
| Data and figures created by the authors (`data/`, `results/`, `figures/`) | CC BY 4.0 | `LICENSE-DATA` |
...
directory layout and do not run in this repository's layout; see 9-11.
...
```
 ...
 (5) generate_reports.py          eval + replay + sideeffect_raw.json + exp3_timing_raw.csv
     ...
     derive_table5_7_10.py        eval + replay   ->  table5_bias_by_api.csv,
                                                      table7_substitute_by_group.csv,
                                                      table10_intercepted_by_category.csv
                                                      (written for v1.0.1; see 9-1)
```

`derive_table5_7_10.py` takes its input and output directories as arguments:

```bat
python scripts\python\derive_table5_7_10.py --eval-dir <dir of *_eval.json> ^
  --replay-dir <dir of *_result.json> --out-dir results\csv
```
...
unchanged, as it was used; see 9-12.
...
| Table 5 | `tab:bias` | Row 4 (89,877 calls, 6.64 percentage points): `results/csv/table5_bias_by_api.csv`. Rows 1–3 state direction and qualitative magnitude only and have no data file. See 9-1 | `derive_table5_7_10.py` (re-derivation written for v1.0.1; see 9-1) |
...
| Table 7 | `tab:substitute` | `results/csv/table7_substitute_by_group.csv` (the eight groups and the total 39,177). See 9-1 | `derive_table5_7_10.py` (re-derivation written for v1.0.1; see 9-1) |
...
| Table 10 | `tab:safety` | "Intercepted and redirected" column: `results/csv/table10_intercepted_by_category.csv`. "Escapes" and "background" columns: `results/csv/exp2_file.csv`, `exp2_registry.csv`, `exp2_network.csv`, `results/csv/sideeffect_raw.json` → `results/figures/table_exp2.xlsx` | First column: `derive_table5_7_10.py` (re-derivation written for v1.0.1; see 9-1). Other columns: `run_exp1.ps1` → `generate_reports.py` → `generate_tables_and_draft.py` |
...
| Figure 1 | `fig:arch` | Figure: `figures/fig-architecture.pdf`. No backing data file (schematic diagram; see 9-2) | — (see 9-2) |
| Figure 2 | `fig:handlechain` | Figure: `figures/fig-handlechain.pdf`. No backing data file (schematic diagram; see 9-2) | — (see 9-2) |
| Figure 3 | `fig:outcome` | Figure: `figures/fig-outcome.pdf`. No backing data file (schematic diagram; see 9-2) | — (see 9-2) |
| Figure 4 | `fig:environment` | Figure: `figures/fig-environment.pdf`. No backing data file (schematic diagram; see 9-2) | — (see 9-2) |
| Figure 5 | `fig:exp1family` | Figure: `figures/fig-exp1-family.pdf`. PNG version: `results/figures/fig_exp1_family.png` ← `results/csv/exp1_summary.csv` | PNG version: `generate_figures.py` (see 9-2 for the PDF) |
| Figure 6 | `fig:exp1category` | Figure: `figures/fig-exp1-category.pdf`. PNG version: `results/figures/fig_exp1_category.png` ← `results/csv/exp1_detail.csv` | PNG version: `generate_figures.py` (see 9-2 for the PDF) |
| Figure 7 | `fig:exp3` | Figure: `figures/fig-exp3-scatter.pdf`. PNG version: `results/figures/fig_exp3_scatter.png` ← `results/csv/exp3_timing.csv` ← `results/csv/exp3_timing_raw.csv` | PNG version: `generate_exp3.py` (raw file written by `run_exp1.ps1`; see 9-2 for the PDF) |
| Figure 8 | `fig:exp7` | Figure: `figures/fig-exp7-confusion.pdf`. PNG version: `results/figures/fig_exp7_v2_confusion.png`; the accuracies it shows are the `acc_5fold` values in `results/csv/exp7_classifier_v2.csv` | PNG version: `exp7_classifier_v2.py` (see 9-2 for the PDF) |
...
**9-1. Table 5, Table 7 and the first column of Table 10 are re-derived, not
produced by the original script.** The script that first computed the counts
in Table 5 (`tab:bias`, row 4: 89,877 calls, 6.64 percentage points),
Table 7 (`tab:substitute`: eight groups, 39,177 calls) and the "intercepted and
redirected" column of Table 10 (`tab:safety`: file 122,841, registry 462,839,
network 65) was not kept. The API sets and the classification criteria behind
these counts were, however, recorded, and the counts can be re-derived from the
preserved evaluation logs. `scripts/python/derive_table5_7_10.py`, written for
release v1.0.1, does this and writes `results/csv/table5_bias_by_api.csv`,
`results/csv/table7_substitute_by_group.csv` and
`results/csv/table10_intercepted_by_category.csv`, which are included. The
re-derivation reproduces the published values exactly. The API sets and
criteria it uses are listed at the top of the script. Its inputs, the per-sample
evaluation outputs (`<sha256>_eval.json`) and replay results
(`<sha256>_result.json`), are not included because they carry the virtual
machine's SID (see 9-9); they are regenerated by stages 2 and 3 of section 6.
The per-channel counts quoted in the Limitations section (named kernel objects,
process/thread/shell launches, service-control calls, Winsock calls,
block-listed terminations, COM activations, device-control requests) are not
covered by this re-derivation, and no script in this repository computes them.

**9-2. Figures 1–4 are schematic; the figure PDFs are included, the scripts
that drew them are not.** `fig:arch`, `fig:handlechain`, `fig:outcome` and
`fig:environment` are architecture and procedure diagrams drawn for the paper.
They have no backing data file. The figures themselves are included as PDF in
`figures/`, together with Figures 5–8 as they appear in the paper. The source
from which the PDFs were drawn is not part of this repository; the PDF metadata
name Matplotlib as the creating tool. For Figures 5–8, the PNG versions in
`results/figures/` are the outputs of the scripts listed in section 8; the PDFs
in `figures/` are the versions used in the paper.

**9-3. The 95% bootstrap intervals cannot be reproduced to the digit.** The
intervals reported for the Layer-1 Behavior Reproduction Rate
(`[71.02, 76.75]` for 74.04%, `[68.15, 73.88]` for the conservative 71.15%) were
obtained by resampling whole samples with replacement, stratified by family,
with 10,000 replicates; the per-family bootstrap intervals of Table 8 were
obtained the same way. **The random seed used for the published values was not
recorded**, so those exact values cannot be regenerated; repeating the
resampling with other seeds moves the interval endpoints by about 0.1
percentage point for the overall rate, and by up to about 0.2 percentage point
across all the bootstrap intervals reported (overall, conservative and
per-family). The input (`results/csv/exp1_detail.csv`, per-sample `l1_success`
and `l1_total`) and the method are both given in this repository and in the
paper. No bootstrap script is included. The Wilson intervals reported alongside them
(`[73.97, 74.12]` and `[71.07, 71.22]`) are deterministic and reproducible:
`wilson_ci()` in `generate_reports.py` computes them from the counts
(1,001,634 of 1,352,778 calls for the headline rate; 1,001,634 − 39,177 =
962,457 of 1,352,778 for the conservative rate).
...
**9-10. Two API categories in the signature database are never exercised by
...
**9-11. The Python post-processing scripts ship without unit tests.** The
...
**9-12. `batch_convert.py` cannot convert all 500 samples on its own.** It
